...
- [Amazon's DynamoDB — 10 years later](https://www.amazon.science/latest-news/amazons-dynamodb-10-years-later) - Deep dive into DynamoDB's evolution over 10 years.
...
- [The Three DynamoDB Limits You Need to Know](https://www.alexdebrie.com/posts/dynamodb-limits/) - Essential guide to understanding DynamoDB's key limitations.
...
- [Alternator](https://github.com/scylladb/scylla/blob/master/docs/alternator/alternator.md) - a Scylla feature adding compatibility with Amazon DynamoDB. It can be used [locally with Docker](https://hub.docker.com/r/scylladb/scylla/) or in production.
...
- [DynamoDB OneTable](https://github.com/sensedeep/dynamodb-onetable) - DynamoDB library that makes single table designs using Node.js much easier via a high-level type-safe API.
...
- [dynamodb-shell](https://github.com/awslabs/dynamodb-shell) - A simple CLI for DynamoDB modeled on isql and the MySQL CLIs. a simple CLI for DynamoDB modeled on isql, and the MySQL CLIs. 
- [NoSQL Architect](https://volisoft.org/ddb.html) - A fully automated schema generator and cost optimization tool for single-table designs in DynamoDB.
...
- [DynoTable](https://dynotable.com) - Cross-platform desktop DynamoDB GUI whose SQL Workbench runs SELECT queries with JOINs, GROUP BY and aggregations over live tables, previews Scan vs Query with an RCU estimate before each run, and exposes an MCP server so Claude Code, Cursor and Codex can read your schema and items.
